...
func _build_top_bar(parent: Control) -> void:
	...
	zoom_out_btn.text = "−"
	zoom_out_btn.custom_minimum_size.x = 28
	zoom_out_btn.tooltip_text = "Zoom out (−)"
	zoom_out_btn.pressed.connect(func() -> void: _zoom_at(1.0 - ZOOM_STEP * 2, size * 0.5))
	zoom_out_btn.mouse_filter = Control.MOUSE_FILTER_STOP
	hbox.add_child(zoom_out_btn)

	_zoom_lbl = Label.new()
	_zoom_lbl.text = "100%"
	_zoom_lbl.custom_minimum_size.x = 48
	...
	zoom_in_btn.text = "+"
	zoom_in_btn.custom_minimum_size.x = 28
	zoom_in_btn.tooltip_text = "Zoom in (+)"
	zoom_in_btn.pressed.connect(func() -> void: _zoom_at(1.0 + ZOOM_STEP * 2, size * 0.5))
	zoom_in_btn.mouse_filter = Control.MOUSE_FILTER_STOP
